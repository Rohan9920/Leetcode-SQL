...
CREATE TABLE Stocks
    (stock_name varchar2(12), operation varchar2(4), operation_day int, price int)
;

INSERT ALL 
    INTO Stocks (stock_name, operation, operation_day, price)
...
with temp as
(select stock_name, price, case when operation = 'Buy' then -1 else 1 end as unit
from stocks)
select stock_name, sum((price * unit))as capital_gain_loss from temp
group by stock_name;

-- Solution 2: (Using lead function)

with temp as
(select stock_name,operation_day ,lead(price,1,0) over (partition by
stock_name order by operation_day)- price as diff, row_number() over (partition by
stock_name order by operation_day) as rn from stocks)
select stock_name, sum(diff) as capital_gain_loss from temp where mod(rn,2)!=0 group by stock_name 
order by stock_name;
