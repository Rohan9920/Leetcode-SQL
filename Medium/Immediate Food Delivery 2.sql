...
;
INSERT ALL 
    INTO Delivery (delivery_id, customer_id, order_date, customer_pref_delivery_date)
...
with temp as
(select d.*, row_number() over (partition by customer_id order by order_date) as rn from delivery d)
select round(avg(case when order_date = customer_pref_delivery_date then 1 else 0 end),2) * 100 as immediate_percentage from temp
where rn=1;